...
- Run `npm run typecheck` (or `tsc -p tsconfig.app.json --noEmit`) after code changes to catch all TypeScript type errors.
- Always run `npx ng build --configuration development` and `npm run build` after template/code changes to verify Angular AOT template compilation.
